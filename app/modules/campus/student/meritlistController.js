	...
	var meritlistController= function ($scope,$location,$rootScope,$injector,modalService, $routeParams,$notification,dataService,uiGridConstants) {
		...
		$scope.meritList = {
			...
			columnDefs: [
				...
				{ 
					name:'address',
					filterHeaderTemplate: '<input id="address" class="form-control" ng-change="grid.appScope.filter(\'address\', address, \'student_view\', \'studentList\',true,grid.appScope.studParams)" ng-model="address" placeholder="Address">'
				},
				{ 
					name:'email_id',width:120,
					filterHeaderTemplate: '<input id="email_id" class="form-control" ng-change="grid.appScope.filter(\'email_id\', email_id, \'student_view\', \'studentList\',true,grid.appScope.studParams)" ng-model="email_id" placeholder="Email ID">',
				},
				{ 
					name:'Marks',width:100,
					filterHeaderTemplate: '<input id="education" class="form-control" ng-change="grid.appScope.filter(\'education\', education, \'student_view\', \'studentList\',true,grid.appScope.studParams)" ng-model="education" placeholder="Marks">',
					cellTemplate : '<span>{{row.entity.education[0].marks}}</span>'
				},
				{ 
					name:'Percentage',width:120,
					filterHeaderTemplate: '<input id="education" class="form-control" ng-change="grid.appScope.filter(\'education\', education, \'student_view\', \'studentList\',true,grid.appScope.studParams)" ng-model="education" placeholder="Percentage">',
					cellTemplate : '<span>{{row.entity.education[0].percentage}}</span>'
				},
				
				{ 
					name:'class_name',enableSorting: false ,
					filterHeaderTemplate: '<select id="name" class="form-control" ng-change="grid.appScope.filter(\'class_id\', class_id, \'student_view\', \'studentList\',true, grid.appScope.studParams)" ng-model="class_id" ng-options="item.class_name as item.class_name for item in grid.appScope.classList">',
				...
				},
				{
					name:'Manage', 
					enableSorting: false, 
					filterHeaderTemplate: '<select id="status" class="form-control" ng-change="grid.appScope.filter(\'status\', status, \'student_view\', \'studentList\',false,grid.appScope.studParams)" ng-model="status">'
